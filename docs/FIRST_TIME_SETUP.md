...
| `/merge-court-order-to-docket` | Configured administrator, Judge, or Attorney General | Run in either active side of the relationship: in the Court Order, enter only `docket_id:DCK-######`; in the Docket Forum post, enter only `court_order_id:COR-######`. It merges immediately and keeps the source Court Order open. |
...
| Court Order must join a prosecuted case | Use `/merge-court-order-to-docket` from either the private Court Order ticket with its destination `DCK-######`, or the destination Docket post with its source `COR-######`. The ID-based merge keeps the Court Order open; use `/import-court-order` in the Docket when leadership specifically wants to close the source after a successful import. |
